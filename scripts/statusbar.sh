...
audio_volume=$(amixer -c $ALSACHANNEL get Master | awk '/Mono.+/ {print $6=="[off]" ? "🔇 ": "🔊 "$4}' | tr -d [])
mic_volume=$(amixer -c $ALSACHANNEL get Mic | awk '/Front.Left:.+/ {print $7=="[off]" ? "🎤MUTE ": "🎤 "$5}' | tr -d [])

# swaymsg -t get_inputs | grep "identifier"
if [ -f /usr/bin/swaymsg ]; then
